...
#define BASE_RAISE_UNLESS(v) \
	BASE_RAISE_IF(!(v))

#define BASE_RAISE_VERBOSE_IF(v, msg) \
	if(v) {BASE_RAISE(#v " should not be ture: " << msg)}

#define BASE_RAISE_VERBOSE_UNLESS(v, msg) \
	BASE_RAISE_VERBOSE_IF(!(v), msg)
