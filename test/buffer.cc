...
TEST(buffer, should_work)
{
  ...
  EXPECT_EQ(ucc::Ret::OK, buf.write(src_buf.get(), tgt_len));
  EXPECT_EQ(tgt_len, buf.get_total_data_size());
  EXPECT_EQ(ucc::Ret::OK, buf.write(src_buf.get(), tgt_len));
  EXPECT_EQ(mem_len, buf.get_total_data_size());
  EXPECT_NE(ucc::Ret::OK, buf.write(src_buf.get(), 1U));

  auto tgt_buf = std::make_unique<uint8_t[]>(mem_len);

  EXPECT_EQ(ucc::Ret::OK, buf.read(src_buf.get(), tgt_len));
  ...
  tgt_len = 1U;
  EXPECT_NE(ucc::Ret::OK, buf.read(src_buf.get(), tgt_len));

  EXPECT_EQ(ucc::Ret::OK, buf.write_at(src_buf.get(), mem_len, 0));
  ...
  tgt_len = mem_len;
  EXPECT_EQ(ucc::Ret::OK, buf.read_at(src_buf.get(), tgt_len, 0));
